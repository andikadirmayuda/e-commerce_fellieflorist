                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-green-100 text-green-700 animate-pulse">
                                        <i class="bi bi-calendar-event text-green-600 mr-1"></i>
                                        {{ \Carbon\Carbon::now()->translatedFormat('F Y') }}
                                    </span>
                                </div>
                                <p class="text-xs text-orange-600 mt-1">Revenue bulan berjalan</p>
                            </div>
                        </div>
                        <div class="hidden md:block">
                            <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M8 32L20 20L28 28L40 16" stroke="#10B981" stroke-width="3" stroke-linecap="round"
                                    stroke-linejoin="round" />
                                <path d="M40 16V28H28" stroke="#10B981" stroke-width="3" stroke-linecap="round"
                                    stroke-linejoin="round" />
                            </svg>
                        </div>
                    </div>
                    ...
                    <div class="stats-card p-4">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-2xl font-bold text-red-700">Rp
                                    {{ number_format($totalOutflow ?? 0, 0, ',', '.') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="stats-card p-4">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p
                                    class="text-2xl font-bold {{ ($saldoBersih ?? 0) >= 0 ? 'text-green-700' : 'text-red-700' }}">
                                    Rp {{ number_format($saldoBersih ?? 0, 0, ',', '.') }}</p>
            ...
            @if(auth()->user()->hasRole(['owner']))
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 form-enter">
                    ...
                    <div class="chart-card p-6">
                        ...
                        {{-- <div class="text-sm text-gray-500 mb-3">Total unit terjual dari transaksi yang sudah selesai
                        </div> --}}
                        <div class="relative">
                ...
                {{-- Waktu Terbanyak Pemesanan --}}
                <div class="grid grid-cols-1 gap-6 mb-8 form-enter">
                    <div class="chart-card p-6">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-2">
                                <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                                    <i class="bi bi-clock-history mr-2 text-yellow-600"></i>
                                    Waktu Terbanyak Pemesanan Online
                                </h3>
                                <span class="text-xs text-yellow-600 bg-yellow-50 px-2 py-1 rounded">Heatmap</span>
                            </div>
                            <form method="GET" action="{{ url('dashboard') }}" id="orderTimeFilterForm"
                                class="flex items-center gap-2">
                                <label for="order_time_range" class="text-xs text-gray-600 mr-1">Periode:</label>
                                <select name="order_time_range" id="order_time_range"
                                    class="text-xs rounded border-gray-300 focus:ring-pink-500 focus:border-pink-500">
                                    <option value="1d" {{ ($orderTimeRange ?? '30d') == '1d' ? 'selected' : '' }}>Hari Ini
                                    </option>
                                    <option value="7d" {{ ($orderTimeRange ?? '30d') == '7d' ? 'selected' : '' }}>7 Hari
                                    </option>
                                    <option value="30d" {{ ($orderTimeRange ?? '30d') == '30d' ? 'selected' : '' }}>30 Hari
                                    </option>
                                </select>
                            </form>
                            <script>
                                document.getElementById('order_time_range').addEventListener('change', function () {
                                    document.getElementById('orderTimeFilterForm').submit();
                                });
                            </script>
                        </div>
                        <div class="relative">
                            <canvas id="orderTimeChart" class="w-full h-64"></canvas>
                        </div>
                    </div>

                    {{-- Waktu Terbanyak Pengunjung --}}
                    <div class="grid grid-cols-1 gap-6 mb-8 form-enter">
                        <div class="chart-card p-6">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2">
                                    <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                                        <i class="bi bi-clock-history mr-2 text-yellow-600"></i>
                                        Waktu Terbanyak Pengunjung Beli Langsung Ke Toko
                                    </h3>
                                    <span class="text-xs text-yellow-600 bg-yellow-50 px-2 py-1 rounded">Heatmap</span>
                                </div>
                                <form method="GET" action="{{ url('dashboard') }}" id="saleTimeFilterForm"
                                    class="flex items-center gap-2">
                                    <label for="order_time_range" class="text-xs text-gray-600 mr-1">Periode:</label>
                                    <select name="order_time_range" id="sale_time_range"
                                        class="text-xs rounded border-gray-300 focus:ring-pink-500 focus:border-pink-500">
                                        <option value="1d" {{ ($orderTimeRange ?? '30d') == '1d' ? 'selected' : '' }}>Hari Ini
                                        </option>
                                        <option value="7d" {{ ($orderTimeRange ?? '30d') == '7d' ? 'selected' : '' }}>7 Hari
                                        </option>
                                        <option value="30d" {{ ($orderTimeRange ?? '30d') == '30d' ? 'selected' : '' }}>30
                                            Hari</option>
                                    </select>
                                </form>
                                <script>
                                    document.getElementById('sale_time_range').addEventListener('change', function () {
                                        document.getElementById('saleTimeFilterForm').submit();
                                    });
                                </script>
                            </div>
                            <div class="relative">
                                <canvas id="saleTimeChart" class="w-full h-64"></canvas>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mb-8 form-enter">
                            ...
                            <div class="chart-card p-6">
                                ...
                                <!-- Filter Form for Performa Pendapatan -->
                                <form method="GET" action="{{ url('dashboard') }}"
                                    class="mb-6 flex flex-col md:flex-row md:items-end gap-4 md:gap-6 bg-pink-50/60 rounded-xl px-4 py-3 border border-pink-100 shadow-sm">
                                    <div class="flex flex-col gap-2 w-full md:w-auto">
                                        <div class="flex gap-4 items-center">
                                            <label class="inline-flex items-center cursor-pointer">
                                                <input type="radio" name="filter_mode" value="month" id="filter_mode_month"
                                                    class="form-radio text-pink-600" {{ (!request('filter_mode') || request('filter_mode') == 'month') ? 'checked' : '' }}>
                                                <span class="ml-2 text-sm font-semibold text-pink-700">Bulan/Tahun</span>
                                            </label>
                                            <label class="inline-flex items-center cursor-pointer">
                                                <input type="radio" name="filter_mode" value="range" id="filter_mode_range"
                                                    class="form-radio text-pink-600" {{ request('filter_mode') == 'range' ? 'checked' : '' }}>
                                                <span class="ml-2 text-sm font-semibold text-pink-700">Rentang
                                                    Tanggal</span>
                                            </label>
                                        </div>
                                        <div class="text-xs text-gray-500 mt-1">Pilih mode filter: berdasarkan bulan/tahun
                                            atau
                                            rentang tanggal tertentu.</div>
                                    </div>
                                    <div class="flex flex-wrap gap-4 w-full md:w-auto">
                                        <div id="month_year_group" class="flex gap-2 items-end">
                                            <div>
                                                <label for="month"
                                                    class="block text-xs font-semibold text-gray-600 mb-1">Bulan</label>
                                                <select name="month" id="month"
                                                    class="rounded-lg border-gray-300 focus:ring-pink-500 focus:border-pink-500 min-w-[110px]">
                                            ...
                                            <div>
                                                ...
                                                <select name="year" id="year"
                                                    class="rounded-lg border-gray-300 focus:ring-pink-500 focus:border-pink-500 min-w-[90px]">
                                        ...
                                        <div id="range_group" class="flex gap-2 items-end">
                                            <div>
                                                <label for="start_date"
                                                    class="block text-xs font-semibold text-gray-600 mb-1">Dari
                                                    Tanggal</label>
                                                <input type="date" name="start_date" id="start_date"
                                                    value="{{ request('start_date') }}"
                                                    class="rounded-lg border-gray-300 focus:ring-pink-500 focus:border-pink-500 min-w-[130px]">
                                            </div>
                                            <div>
                                                <label for="end_date"
                                                    class="block text-xs font-semibold text-gray-600 mb-1">Sampai
                                                    Tanggal</label>
                                    ...
                                    <div class="flex gap-2 mt-2 md:mt-0">
                                        <button type="submit"
                                            class="bg-pink-600 hover:bg-pink-700 text-white font-semibold px-5 py-2 rounded-lg transition">Terapkan
                                        </button>
                                        <button type="button" onclick="window.location.href='{{ url('dashboard') }}'"
                                            class="bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold px-5 py-2 rounded-lg transition">Reset
                                        </button>
                                    </div>
                                </form>
                                <script>
                                    document.addEventListener('DOMContentLoaded', function () {
                                        ...
                                        document.querySelectorAll('input[name="filter_mode"]').forEach(el => {
                        ...
                        {{-- Baris 4: Performa Pendapan Pertahun --}}
                        <div class="grid grid-cols-1 gap-6 mb-8 form-enter">
                            <!-- Revenue Performance Chart -->
                            <div class="chart-card p-6">
                                <div class="flex items-center justify-between mb-4">
                                    <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                                        <i class="bi bi-bar-chart-line mr-2 text-pink-600"></i>
                                        Performa Pendapatan Pertahun
                                    </h3>
                                    <span class="text-xs text-pink-600 bg-pink-50 px-2 py-1 rounded">Yearly</span>
                                </div>
                                <div class="relative">
                                    <canvas id="yearlyRevenueChart" class="w-full h-64"></canvas>
                                </div>
                            </div>
            @endif
                                            ...
                                            <div class="flex items-center p-3 bg-red-50 border border-red-200 rounded-lg">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <p class="text-xs text-red-600">
                                                        {{ data_get($product, 'name', '-') }} tersisa
                                                        {{ data_get($product, 'current_stock', 0) }} unit
                                                    </p>
                                                </div>
                                ...
                                <div class="space-y-3">
                                    @if(auth()->user()->hasRole(['owner', 'admin']))
                                        ...
                                        <a href="{{ route('bouquets.create') }}"
                                            class="w-full bg-pink-600 hover:bg-pink-700 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center transition-colors">
                                            <i class="bi bi-plus-circle mr-2"></i>
                                            Tambah Bouquet
                                        </a>
                                        ...
                                        <a href="{{ route('cashflow.index') }}"
                                            class="w-full bg-yellow-600 hover:bg-yellow-700 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center transition-colors">
                                            <i class="bi bi-cash-stack mr-2"></i>
                                            Lihat Cash Flow
                                        </a>


                                        {{-- Lihat Produk Ready Stock & Performance --}}
                                        <a href="#ready-stock-performance"
                                            class="w-full bg-gray-800 hover:bg-gray-900 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center transition-colors">
                                            <i class="bi bi-flower2 mr-2"></i>
                                            Produk Ready Stock & Performance
                                        </a>

                                        {{-- Lihat Bouquet Ready Stock & Performance --}}
                                        <a href="#bouquet-ready-stock-performance"
                                            class="w-full bg-pink-600 hover:bg-pink-700 text-white font-medium py-3 px-4 rounded-lg flex items-center justify-center transition-colors">
                                            <i class="bi bi-flower2 mr-2"></i>
                        ...
                        <div class="notification-card p-6 form-enter" id="ready-stock-performance">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                                ...
                                <div class="flex items-center space-x-4">
                                    ...
                                    <a href="{{ url('/product-fellieflorist') }}" target="_blank"
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-blue-600 mb-2">
                                            Link ini dapat dibagikan kepada pelanggan untuk melihat produk ready stock
                                            tanpa
                                            perlu
                                            login:
                                        </p>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td class="text-center">
                                                            <div class="flex flex-col items-center">
                                                                <span
                                                                    class="text-lg font-bold text-gray-800">{{ data_get($product, 'current_stock', 0) }}</span>
                                                                <span
                                                                    class="text-xs text-gray-500">{{ data_get($product, 'base_unit', 'pcs') }}</span>
                                                            </div>
                                                        </td>
                                                        <td class="text-center">
                                                            @php
                                                            ...
                                                            @if($stock > $minStock * 2)
                                                                <span
                                                                    class="inline-flex items-center px-2 py-1 bg-green-100 text-green-700 rounded text-xs">
                                                                    <i class="bi bi-check-circle mr-1"></i>
                                                                    Baik
                                                                </span>
                                                            @elseif($stock > $minStock)
                                                                <span
                                                                    class="inline-flex items-center px-2 py-1 bg-yellow-100 text-yellow-700 rounded text-xs">
                                                                    <i class="bi bi-exclamation-circle mr-1"></i>
                                                                    Sedang
                                                                </span>
                                                            @else
                                                                <span
                                                                    class="inline-flex items-center px-2 py-1 bg-red-100 text-red-700 rounded text-xs">
                                                        ...
                                                        <td class="text-center">
                                                            <div class="flex flex-col items-center">
                                                                ...
                                                                <span class="text-xs text-{{ $color }}-600 font-medium">
                                                                    <i
                                                                        class="bi {{ $icon }} {{ $performance === 'Laris' ? 'text-orange-500' : '' }}"></i>
                                                                    {{ $performance }}
                                                                </span>
                                                                ...
                                                                <span class="text-xs text-gray-500 mt-1">
                                                                    {{ $product->sold_detail_string ? $product->sold_detail_string . ' terjual' : '0 terjual' }}
                                                                </span>
                                                            </div>
                                                        </td>
                                                        <td class="text-center">
                                                ...
                                                <tr>
                                                    <td colspan="6" class="text-center py-8">
                                                        <div class="flex flex-col items-center">
                                                            ...
                                                            <p class="text-gray-500">Tidak ada produk ready stock saat ini
                                                            </p>
                                                            <a href="{{ route('inventory.index') }}"
                            ...
                            <!-- Bouquet Ready Stock & Performance -->
                            <div class="notification-card p-6 form-enter mt-8" id="bouquet-ready-stock-performance">
                                <div class="flex items-center justify-between mb-4">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <div class="flex items-center">
                                                                ...
                                                                <div>
                                                                    <p class="font-medium text-gray-800">
                                                                        {{ data_get($bouquet, 'name', '-') }}
                                                                    </p>
                                                                    <p class="text-xs text-gray-500">
                                                                        {{ data_get($bouquet, 'code', '-') }}
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <span
                                                                class="inline-flex items-center px-2 py-1 bg-pink-50 text-pink-700 rounded text-xs">
                                                        ...
                                                        <td class="text-center">
                                                            <div class="flex flex-col items-center">
                                                                <span
                                                                    class="text-lg font-bold text-gray-800">{{ data_get($bouquet, 'current_stock', 0) }}</span>
                                                                <span
                                                                    class="text-xs text-gray-500">{{ data_get($bouquet, 'base_unit', 'pcs') }}</span>
                                                            </div>
                                                        </td>
                                                        <td class="text-center">
                                                            @php
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="6" class="text-center py-8">
                                                        <div class="flex flex-col items-center">
                                                            <i class="bi bi-inbox text-4xl text-gray-300 mb-2"></i>
                                                            <p class="text-gray-500">Tidak ada bouquet ready stock saat ini
                                                            </p>
                                                        </div>
                                                    </td>
                                                </tr>
                    ...
                    <script>
                        ...
                        const saleTimeCtx = document.getElementById('saleTimeChart').getContext('2d');


                        // Direct Sale Time Heatmap Chart (Stacked Bar)
                        const saleTimeData = {!! json_encode($directSaleTimeHeatmap) !!};
                        // saleTimeData: { labels: [jam], days: [hari], data: [[minggu, senin, ...], ...per jam] }
                        const saleTimeDatasets = saleTimeData.days.map((day, dayIdx) => {
                            return {
                                label: day,
                                data: saleTimeData.data.map(row => row[dayIdx]),
                                backgroundColor: [
                                    '#F59E42', // Minggu
                                    '#3B82F6', // Senin
                                    '#10B981', // Selasa
                        ...
                        new Chart(saleTimeCtx, {
                            type: 'bar',
                            data: {
                                labels: saleTimeData.labels,
                                datasets: saleTimeDatasets
                            },
                            options: {
                                responsive: true,
                        ...
                        // Order Time Heatmap Chart (Stacked Bar)
                        const orderTimeData = {!! json_encode($orderTimeHeatmap) !!};
                        // orderTimeData: { labels: [jam], days: [hari], data: [[minggu, senin, ...], ...per jam] }
                        // Transpose data: per hari, data per jam
                        const orderTimeDatasets = orderTimeData.days.map((day, dayIdx) => {
                            return {
                                label: day,
                                data: orderTimeData.data.map(row => row[dayIdx]),
                                backgroundColor: [
                                    '#F59E42', // Minggu
                                    '#3B82F6', // Senin
                                    '#10B981', // Selasa
                        ...
                        new Chart(orderTimeCtx, {
                            type: 'bar',
                            data: {
                                labels: orderTimeData.labels,
                                datasets: orderTimeDatasets
                            },
                            options: {
                                responsive: true,
                                maintainAspectRatio: false,
                        ...
                        // Yearly Revenue Chart (Bar/Line)
                        const yearlyRevenueCtx = document.getElementById('yearlyRevenueChart').getContext('2d');
                        const yearlyRevenueData = {!! json_encode($yearlyRevenueChartData) !!};
