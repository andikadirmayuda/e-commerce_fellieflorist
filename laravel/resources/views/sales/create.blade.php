...
<div id="productModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-30 hidden">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        <button id="closeProductModal"
            class="absolute top-3 right-3 text-gray-400 hover:text-pink-600 text-xl">&times;</button>
        ...
        <button id="modalAddToCart"
            class="w-full py-2 bg-pink-600 hover:bg-pink-700 text-white font-semibold rounded-lg transition">Tambah ke
            Keranjang</button>
    </div>
</div>
<x-app-layout>
    ...
    <div class="py-6 sm:py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-[3fr,2fr] gap-8 items-start">
                <!-- Error Messages -->
                @if ($errors->any())
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div class="space-y-6">
                                    <div>
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">Untuk mengirim link invoice ke customer
                                        </p>
                                    </div>
                            ...
                            <div class="border-t border-gray-200 pt-8">
                                                        ...
                                                        <div class="grid grid-cols-2 gap-2" id="productsGrid">
                                                            @foreach($products as $product)
                                                                <button type="button"
                                                                    ...
                                                                    data-original-name="{{ $product->name ?? '' }}" {{ $product->current_stock == 0 ? 'disabled' : '' }}>
                                                                    ...
                                                                    @else
                                                                        <span
                                                                            class="w-8 h-8 flex items-center justify-center bg-gray-100 rounded border border-gray-200 text-gray-400"><i
                                                                                class="bi bi-box"></i></span>
                                                                    @endif
                                                                    <div class="flex flex-col flex-1 min-w-0">
                                                                        <span
                                                                            class="font-medium text-gray-900 truncate">{{ $product->name }}</span>
                                                                        <span
                                                                            class="text-xs text-gray-500 truncate">{{ $product->code ?? '-' }}</span>
                                                                        <span
                                                                            class="text-sm text-pink-600 font-semibold">{{ $product->category->name ?? 'Tanpa Kategori' }}</span>
                                                                    </div>
                                                                    <div class="flex flex-col items-end ml-2">
                                                                        ...
                                                                        @else
                                                                            <span
                                                                                class="text-xs font-semibold text-red-600 bg-red-100 rounded px-2 py-0.5">Habis</span>
                                                                        @endif
                                                                    </div>
                                                                </button>
                                                            @endforeach
                                ...
                                <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex w-full rounded-lg shadow-sm overflow-hidden">
                                            <button type="button" id="quantityMinusBtn"
                                                class="inline-flex items-center justify-center w-12 h-12 border border-gray-300 bg-white text-gray-700 rounded-l-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-pink-500 flex-shrink-0">
                                            ...
                                            <button type="button" id="quantityPlusBtn"
                                                class="inline-flex items-center justify-center w-12 h-12 border border-gray-300 bg-white text-gray-700 rounded-r-lg hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-pink-500 flex-shrink-0">
                                ...
                                <div
                                    ...
                                    <div class="flex items-start space-x-3">
                                        ...
                                        <div class="flex-grow">
                                            <label class="block text-sm font-medium text-gray-700 mb-3">
                                                <i class="bi bi-lightning-charge mr-2 text-orange-500"></i>Cari Cepat
                                                dengan
                                                Kode Produk
                                            </label>
                                            <div class="relative">
                                            ...
                                            <tr id="emptyRow">
                                                <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                                    <div class="flex flex-col items-center">
                                                        ...
                                                        <p class="text-lg font-medium text-gray-900 mb-2">Keranjang
                                                            kosong
                                                        </p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4">
                        <input type="text" id="sidebarProductSearch"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg mb-4 focus:outline-none focus:ring-2 focus:ring-pink-500"
                            placeholder="Cari produk...">
                        <div class="max-h-[60vh] overflow-y-auto" id="sidebarProductList">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                @foreach($products as $product)
                                    <div class="bg-gray-50 rounded hover:bg-pink-50 transition cursor-pointer sidebar-product-item p-3 flex flex-col items-start justify-between h-full"
                                        data-name="{{ strtolower($product->name) }}"
                                        data-code="{{ strtolower($product->code) }}" data-id="{{ $product->id }}">
                                        <div class="font-medium text-gray-900 truncate w-full">{{ $product->name }}</div>
                                        <div class="text-xs text-gray-500 w-full truncate">Kode: {{ $product->code ?? '-' }}
                                        </div>
                                        <div class="text-xs text-gray-400 w-full truncate mb-2">Stok:
                                            {{ $product->current_stock }}</div>
                                        <button type="button"
                                            class="mt-auto px-3 py-1 bg-pink-600 hover:bg-pink-700 text-white text-xs rounded focus:outline-none select-product-btn w-full"
                                            data-id="{{ $product->id }}">Pilih</button>
                                    </div>
                                @endforeach
                            </div>
                        </div>
