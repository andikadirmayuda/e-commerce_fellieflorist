...
<head>
    ...
    <style>
        ...
        /* Chip button styling */
        .chip-btn.active {
            background: linear-gradient(135deg, #f2527d, #f25270);
            color: white;
            border-color: #f2527d;
...
<body class="min-h-screen gradient-bg text-black flex flex-col font-sans overflow-x-hidden">
                    ...
                    <nav class="flex items-center justify-center">
                        <div class="flex items-center justify-center space-x-2 sm:space-x-4 md:space-x-8">
                            <a href="{{ route('public.flowers') }}"
                                class="px-2 sm:px-4 py-1.5 text-center {{ $activeTab === 'flowers' ? 'nav-tab nav-hover-effect group relative items-center space-x-2 px-3 py-2 rounded-xl transition-all duration-300 bg-gradient-to-r from-[#f2527d] to-[#f25270] shadow-lg nav-active-gradient' : 'text-[#f2527d] hover:bg-[#f2527d]/10' }}">
                                <span
                            ...
                            <a href="{{ route('public.bouquets') }}"
                                class="px-2 sm:px-4 py-1.5 text-center {{ $activeTab === 'bouquets' ? 'nav-tab nav-hover-effect group relative items-center space-x-2 px-3 py-2 rounded-xl transition-all duration-300 bg-gradient-to-r from-[#f2527d] to-[#f25270] shadow-lg nav-active-gradient' : 'text-[#f2527d] hover:bg-[#f2527d]/10' }}">
                                <span
        ...
        <div class="mb-8 flex flex-col items-center">
            ...
            @if($bouquetCategories->count() > 0)
                <div class="flex flex-wrap gap-3 justify-center mb-4">
                    <button type="button"
                        class="chip-btn px-6 py-3 rounded-full border-2 border-[#f2527d] bg-white text-[#f2527d] text-sm font-semibold shadow-md hover:shadow-lg hover:bg-[#f2527d]/10 transition-all duration-200 active"
                        data-category="" onclick="selectBouquetCategory(this)">
                        <span class="mr-2">💐</span>Semua
                    </button>
                    @foreach($bouquetCategories as $category)
                        <button type="button"
                            class="chip-btn px-6 py-3 rounded-full border-2 border-[#f2527d] bg-white text-[#f2527d] text-sm font-semibold shadow-md hover:shadow-lg hover:bg-[#f2527d]/10 transition-all duration-200"
                        ...
                        <div
                            ...
                            <div class="relative h-48 md:h-80 sm:h-40 mb-3 sm:mb-4 rounded-xl overflow-hidden">
                                ...
                                @if($bouquet->image)
                                    ...
                                    {{-- <button
                                        ...
                                        <span
                                            ...
                                            <i class="bi bi-eye text-[#f2527d] text-xl"></i>
                                        </span>
                                    </button> --}}
                                ...
                                <div class="absolute top-2 sm:top-3 right-2 sm:right-3 flex gap-2 z-30">
                                    ...
                                    <button
                                        onclick="showFullImage('{{ asset('storage/' . $bouquet->image) }}', '{{ $bouquet->name }}')"
                                        class="w-6 sm:w-8 h-6 sm:h-8 bg-white/90 hover:bg-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                        <i class="bi bi-eye text-[#f2527d] text-xs sm:text-sm"></i>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 flex flex-col">
                                <div class="flex items-start justify-between mb-2">
                                    ...
                                    <span
                                        class="ml-2 px-2 py-1 bg-[#f2527d]/10 text-[#f2527d] rounded-full text-[10px] sm:text-xs font-medium whitespace-nowrap flex-shrink-0">
        ...
        $sortedSizes = $bouquet->sizesWithComponents->sortBy(function ($size) use ($sizeOrder) {
            ...
            return $index !== false ? $index : 999; // Put unknown sizes at the end
                                        ...
                                        @foreach($sortedSizes as $size)
                                            <span
                                                class="inline-block px-2 py-1 bg-gradient-to-r from-[#f2527d]/10 to-[#f25270]/10 text-[#f2527d] rounded-full text-[10px] sm:text-xs font-medium">
                                                {{ $size->name }}
                                            </span>
                                        @endforeach
                                    </div>
        ...
        $maxPrice = $pricesWithComponents->max('price');
                                    ...
                                    @if($minPrice && $maxPrice)
                                        <div class="text-center">
                                            @if($minPrice == $maxPrice)
                                                <span class="text-price text-sm sm:text-lg font-bold text-[#f2527d]">
        ...
        $availablePrices = $bouquet->prices->whereIn('size_id', $sizeIdsWithComponents);
            ...
            $firstPrice = $availablePrices->first();
                                    ...
                                    <button onclick="showBouquetDetailPanel({{ $bouquet->id }})"
                                        class="block w-full text-center border-2 border-[#f2527d] text-[#f2527d] hover:bg-[#f2527d]/10 font-semibold py-1.5 sm:py-2 px-3 sm:px-4 rounded-xl transition-all duration-200 text-xs sm:text-sm">
    ...
    <button id="scrollToBottomBtn" title="Scroll ke bawah"
        class="fixed top-1/2 right-6 -translate-y-1/2 z-50 bg-[#f2527d] text-white rounded-full shadow-lg p-3 flex items-center justify-center transition-all duration-300 opacity-0 pointer-events-none hover:bg-[#f25270] focus:outline-none"
