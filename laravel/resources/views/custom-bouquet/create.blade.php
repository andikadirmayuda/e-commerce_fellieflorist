...
<head>
    ...
    <title>Custom Bouquet | Fellie Florist</title>
    ...
    <style>
        ...
        .gradient-bg {
            background: linear-gradient(135deg, #f285a2 0%, #FFFFFF 50%, rgb(248, 180, 200) 100%);
        }
        ...
        .chip-btn.active {
            background: linear-gradient(135deg, #f2527d, #f2527d);
            color: #fff;
            border-color: #f2527d;
            transform: translateY(-2px);
        }
...
<body
                    ...
                    <div class="flex items-center">
                        <a href="{{ route('public.flowers') }}" class="flex items-center">
                            <div>
                                ...
                                <p class="text-[7px] sm:text-xs text-gray-500">Supplier Bunga Palembang</p>
                            </div>
                        </a>
                    </div>
                    ...
                    <nav class="flex items-center justify-center">
                        <div class="flex items-center justify-center space-x-2 sm:space-x-4 md:space-x-8">
                            <a href="{{ route('public.flowers') }}"
                                class="px-2 sm:px-4 py-1.5 text-center text-[#f2527d] hover:bg-[#f2527d]/10 rounded-xl transition-all duration-300">
                            ...
                            <a href="{{ route('custom.bouquet.create') }}"
                                class="px-2 sm:px-4 py-1.5 text-center nav-tab nav-hover-effect group relative items-center space-x-2 px-3 py-2 rounded-xl transition-all duration-300 bg-gradient-to-r from-[#f2527d] to-[#f2527d] text-white shadow-lg nav-active-gradient">
    ...
    <div class="w-full max-w-6xl mx-auto px-4 py-6">
        ...
        <div class="mb-6 horizontal-builder">
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                <!-- Builder Header -->
                <div class="bg-gradient-to-r from-[#f2527d] to-[#f2527d] text-white p-4 relative overflow-hidden">
                    <!-- Background Pattern -->
                        ...
                        <div class="lg:col-span-1">
                            ...
                            <div class="mb-6">
                                <label class="block text-sm font-semibold text-gray-700 mb-3 flex items-center">
                                    ...
                                    Upload Referensi
                                    <span
                                        class="ml-2 text-xs bg-[#2f2e2dff] text-white px-2 py-1 rounded-full">Opsional</span>
                                </label>
                                <div class="relative">
                                    <input type="file" id="referenceImage" accept="image/*" class="hidden">
                                    <button type="button" id="uploadReferenceBtn"
                                        class="w-full border-2 border-dashed border-[#2f2e2dff] hover:border-[#757575ff] rounded-xl p-4 text-center transition-all duration-300 bg-gradient-to-br from-purple-50 to-pink-50 hover:from-purple-100 hover:to-pink-100 group">
                                        <div class="text-[#2f2e2dff] group-hover:text-[#757575ff]">
        ...
        <div class="mb-6 flex items-center justify-center gap-4 text-sm">
            <span class="bg-[#f2527d] text-white px-3 py-1 rounded-full">
                <i class="bi bi-palette2 mr-1"></i>
                Draft ID: #{{ $customBouquet->id }}
            </span>
            <span class="bg-[#f2527d] text-white px-3 py-1 rounded-full font-semibold" id="totalPrice">
                ...
                <div class="flex gap-3">
                    ...
                    <div class="flex-1 relative">
                        ...
                        <input type="text" id="searchInput" placeholder="{{'Cari bunga Custom impian Anda...'  }}"
                            class="w-full pl-12 pr-4 py-4 text-lg border-2 border-[#f2527d] rounded-2xl focus:ring-2 focus:ring-[#f2527d] focus:border-[#f2527d] focus:outline-none shadow-lg transition-all duration-200 bg-white/90"
                            oninput="filterItems()">
                    </div>
                    ...
                    <div class="relative">
                        <select id="priceFilterSelect"
                            class="appearance-none bg-white/90 border-2 border-[#f2527d] rounded-2xl px-4 py-4 pr-10 text-gray-700 focus:ring-2 focus:ring-[#f2527d] focus:border-[#f2527d] focus:outline-none shadow-lg transition-all duration-200 cursor-pointer"
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#f2527d] to-[#f2527d] px-6 py-4 border-b border-gray-100">
            ...
            <!-- Products Grid -->
            <div class="p-6">
                <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6 text-center">
                    {{-- <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4"
                        id="productsGrid"> --}}
                        @foreach($products as $product)
                            @php
                                $customPrices = $product->prices->filter(function ($price) {
                                    return in_array($price->type, ['custom_tangkai', 'custom_ikat', 'custom_khusus']);
                                });

                                // Prioritaskan tipe harga dalam urutan tertentu
                                $priceTypes = ['custom_tangkai', 'custom_ikat', 'custom_khusus'];
                                $defaultPrice = null;
                            ...
                            @if($defaultPrice && $product->current_stock > 0)
                                <div class="product-card glass-effect card-hover rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer group"
                                    ...
                                    <!-- Product Image -->
                                    <div
                                        class="relative aspect-w-1 aspect-h-1 bg-gradient-to-br from-[#F5F5F5] to-[#f2527d]/10 rounded-t-2xl overflow-hidden">
                                        @if($product->image)
                                            ...
                                            <button
                                                ...
                                                <i class="bi bi-eye text-[#f2527d] text-sm"></i>
                                            </button>
                                        @else
                                            <div
                                                class="w-full h-32 bg-gradient-to-br from-[#f2527d]/10 to-[#f2527d]/10 flex items-center justify-center">
                                                <span class="text-4xl text-[#f2527d]">🌸</span>
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <div class="p-4">
                                        ...
                                        <p class="text-xs text-[#f2527d] mb-2 font-medium">
                                            {{ $product->category->name ?? 'Uncategorized' }}
                                        </p>
                                        <!-- Stock Info -->
                                        <div class="text-center items-center justify-between mb-3">
                                        ...
                                        <!-- Add Button -->
                                        <button
                                            class="w-full mt-3 btn-add-bouquet text-white bg-[#f2527d] text-sm py-2 px-3 rounded-xl transition-all duration-200 add-product-btn"
    ...
    <div id="productModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            <!-- Modal Header -->
            <div class="bg-gradient-to-r from-[#f2527d] to-[#f25270] text-white p-4 rounded-t-xl">
            ...
            <div class="p-6">
                ...
                <div class="flex space-x-3">
                    ...
                    <button type="button" id="addToBuilderBtn"
                        class="flex-1 bg-[#f25270] hover:bg-[#f2527d] text-white py-2 px-4 rounded-md transition-colors">
                        Tambah ke Builder
                    </button>
                </div>
            </div>
    ...
    <button id="scrollToBottomBtn" title="Scroll ke bawah"
        class="fixed top-1/2 right-6 -translate-y-1/2 z-50 bg-[#f2527d] text-white rounded-full shadow-lg p-3 flex items-center justify-center transition-all duration-300 opacity-0 pointer-events-none hover:bg-[#f2527d] focus:outline-none"
    ...
    `;
            ...
            const priceTypes = ['custom_tangkai', 'custom_ikat', 'custom_khusus'];
    ...
    `).join('');
        ...
        async function addToBuilder() {
            ...
            // Validasi tipe harga
            if (!['custom_tangkai', 'custom_ikat', 'custom_khusus'].includes(selectedPriceType)) {
        ...
        function displaySelectedItems(items) {
            ...
            <div class="text-center py-8 text-gray-500">
                ...
                <div class="mt-4 flex justify-center">
                    <div class="bg-gradient-to-r from-[#757575ff] to-[#2f2e2dff] rounded-full px-4 py-2">
                        <span class="text-xs text-white font-medium">Mulai membangun bouquet impian Anda</span>
                    </div>
                </div>
            </div>
        `;
        ...
        async function handleRibbonColorChange(e) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showNotification('Berhasil Dipilih', 'success');
                } else {
                    // Revert UI if failed
                    document.querySelectorAll('.ribbon-color-btn').forEach(btn => {
                        btn.classList.remove('selected-ribbon');
